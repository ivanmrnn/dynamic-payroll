...
func generateResponsibilitiesForRole(role *Role) {
    ...
    for responsibilityName, menuInfo := range responsibilities {
        ...
        createResponsibility(specificResponsibilityName, menuInfo.MenuName, menuInfo.MenuIcon)
    }
}

// Create a responsibility without associating it with a role
func createResponsibility(responsibilityName, menuName, menuIcon string) {
    var menu MenuName
    ...
    if err != nil || menu.ID == 0 {
        ...
        uadmin.Save(&menu)
    } else if menu.MenuIcon == "" && menuIcon != "" {
        // Update the menu icon if it's empty and a new one is provided
...
func setResponsibilitiesForLevel(role *Role) {
    responsibilitiesByLevel := map[uint][]string{
        ...
        4: {"View Dashboard", "View Timesheet Log", "View Payroll Log", "View Leave Log", "View Employee", "Manage Employee", "Add Employee", "Manage Identification Format", "Manage Roles", "Manage Salaries", "Manage Deductions", "Manage Company Deductions", "View Audit Logs"},
    }
    ...
    for _, responsibilityName := range responsibilities {
        ...
        if !found {
            // If not found in common map, use the responsibility name for both menu name and responsibility name
            menuInfo = struct {
                MenuName string
                MenuIcon string
            }{responsibilityName, ""}
        ...
        handleResponsibility(&roleResponsibility, responsibilityName, menuInfo.MenuName, menuInfo.MenuIcon)
    }
...
func handleResponsibility(roleResponsibility *RoleResponsibility, responsibilityName, menuName, menuIcon string) {
    var responsibility Responsibility
    err := uadmin.Get(&responsibility, "name = ?", responsibilityName)
    if err != nil || responsibility.ID == 0 {
        createResponsibility(responsibilityName, menuName, menuIcon)
    ...
    for _, r := range roleResponsibility.Responsibility {
        if r.ID == responsibility.ID {
            return // Responsibility already associated, no need to add it again
        }
    }
